...
static PyObject* DNAbi_error = NULL;

static PyObject* DNAbi_version(PyObject* self) {
...
static PyMethodDef DNAbi_methods [] = {
	...
	{ "version", (PyCFunction)DNAbi_version, METH_VARARGS, "Return the version of the DNA library"},
	{NULL, NULL, 0, NULL}
};

// Define the python module
static PyModuleDef DNAbi_module = {
...
PyMODINIT_FUNC PyInit_DNA_bit() {
	PyObject* obj = PyModule_Create(&DNAbi_module);

	if (!obj)
		return NULL;

	DNAbi_error = PyErr_NewException("DNAbi.error", NULL, NULL);
	Py_XINCREF(DNAbi_error);

	if (PyModule_AddObject(obj, "error", DNAbi_error) < 0) {
		Py_XDECREF(DNAbi_error);
		Py_CLEAR(DNAbi_error);
		Py_DECREF(obj);
		return NULL;
	}

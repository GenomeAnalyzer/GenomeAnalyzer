...
static PyObject* DNAst_error = NULL;

static PyObject* DNAst_version(PyObject* self) {
...
static PyObject* DNAb_convert_to_binary(PyObject* self, PyObject* args) {
	...
	seq* array = convert_to_binary(seq_char, seq_size);


	PyObject* pylist = PyList_New(seq_size*2);

	for (long int i = 0; i < seq_size*2; i++)
		PyList_SetItem(pylist, i, PyBool_FromLong(array[i].bit));
...
static PyMethodDef DNAst_methods [] = {
	...
	{ "version", (PyCFunction)DNAst_version, METH_VARARGS, "Return the version of the DNA library"},
	{NULL, NULL, 0, NULL}
};

// Define the python module
static PyModuleDef DNAst_module = {
...
PyMODINIT_FUNC PyInit_DNA_st() {
	PyObject* obj = PyModule_Create(&DNAst_module);

	if (!obj)
		return NULL;

	DNAst_error = PyErr_NewException("DNAst.error", NULL, NULL);
	Py_XINCREF(DNAst_error);

	if (PyModule_AddObject(obj, "error", DNAst_error) < 0) {
		Py_XDECREF(DNAst_error);
		Py_CLEAR(DNAst_error);
		Py_DECREF(obj);
		return NULL;
	}

 ...
 */
int popcount_binary_array(const seq *seq_bin, const long int seq_size){
    int bin_popcount = 0;
    ...
    for (long int i = 0; i < array_size; ++i)
        bin_popcount += __builtin_popcount(seq_bin[i].bit);
...
seq * convert_to_binary(const char* dna_seq, const unsigned size){
    ...
    seq* seq_bin = NULL;
    seq_bin = calloc(size*2, sizeof(seq));
    if(!seq_bin)
...
char* binary_to_dna(seq* bin_dna_seq, const unsigned size){
    ...
    for (unsigned i = 0; i < size; i += 2){
        // nucleotides = A, T, G, C
        int nucl1 = bin_dna_seq[i].bit;
        int nucl2 = bin_dna_seq[i + 1].bit;
 ...
 */
void detecting_genes(const seq* gene, const long int gene_size, gene_map_t* gene_map) {
    ...
    while ((i + 6) <= gene_size) {
        ...
        int nucl1 = gene[i].bit;
        int nucl2 = gene[i + 1].bit;
        int nucl3 = gene[i + 2].bit;
        int nucl4 = gene[i + 3].bit;
        int nucl5 = gene[i + 4].bit;
        int nucl6 = gene[i + 5].bit;
...
void detecting_mutations(const seq*gene_seq, const long int size_sequence,
    ...
    for (long int i = 0; i < size; i += 2) {
        ...
        // each nucleotides can be  A, U, G or C
        int nucl1 = gene_seq[i].bit;
        int nucl2 = gene_seq[i + 1].bit;
...
*/
float calculating_matching_score(const seq seq1 [], const int sequence_size1,
                                 const seq seq2 [], const int sequence_size2) {
    // Check the input argument
    if (!seq1 || !seq2)
    ...
    const seq* s1 = sequence_size1 >= sequence_size2 ? seq1 : seq2;
    const seq* s2 = sequence_size1 >= sequence_size2 ? seq2 : seq1;
    ...
    for (int i = diff_size; i < max_size; ++i) {
        if (s1[i].bit ^ s2[i - diff_size].bit) ++count;
    }

...
// static PyObject* DNA_compress(PyObject* self, PyObject* args) {
// 	int size = 0;
// 	char* dna_seq = NULL;
...
// 	//Get the parameter (char* value)
// 	if (!PyArg_ParseTuple(args, "is", &size, &dna_seq))
// 		return NULL;

// 	// printf("__%d, %c__\n", size, dna_seq[0]);

// 	short* test = compress(size, "AAAAA");
// 	PyObject* pylist, * item;
// 	//TODO: Need to find max element 
...
static PyObject* DNA_generating_amino_acid_chain(PyObject* self, PyObject* args) {
	...
	Py_buffer seq;
	PyObject* seq_buffer = NULL;
	PyObject* obj2 = NULL;

	//Get the parameter (1-dimensional arrays)
	if (!PyArg_ParseTuple(args, "XOXX", &seq_buffer))
		return NULL;

	//Get the array memory seq
	if (PyObject_GetBuffer(seq_buffer, &seq, PyBUF_ANY_CONTIGUOUS | PyBUF_FORMAT) == -1)
		return NULL;

	if (codons.ndim != 1) {
		PyErr_SetString(PyExc_TypeError, "Expecting a 1-dimensional array.");
		PyBuffer_Release(&codons);
		return NULL;
	}

	if (strcmp(seq.format, "h")) {
		PyErr_SetString(PyExc_TypeError, "Expecting a 1-dimensional array of short.");
		PyBuffer_Release(&seq);
		return NULL;
	}

	//Return the boolean value as a Python boolean object
	return Py_BuildValue("O", generating_amino_acid_chain(proteines, seq.shape[0], seq.buf, gene_map, codons_count, codons) ? Py_True : Py_False);
}
...
static PyMethodDef DNA_methods [] = {
	...
	{ "compress", DNA_compress, METH_VARARGS, "Convert a char sequence to a binary sequence"},

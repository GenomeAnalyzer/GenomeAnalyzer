...
for file in glob.glob("fastas/*.fasta"):
    ...
    gen =DNA_bin.detecting_genes(array.array('l',sequence[i]))
    ...
    for j in range (len(gene[i])-1):
        print(str(sequence[i][gene[i][j][0]/31:(gene[i][j][1]/31)]))
        message += "<tr><td>"+str(sequence[i][gene[i][j][0]:(gene[i][j][1])])+ "</td>\n"
        res = (DNA_bin.generating_mRNA(array.array('l',sequence[i][gene[i][j][0]:(gene[i][j][1])])))
        if res:
        ...
        res2 =  DNA_bin.generating_amino_acid_chain(array.array('l',sequence[i][gene[i][j][0]:(gene[i][j][1])]))
        ...
        if res2:
            ...
            message+="<td> none </td>\n"            
        #print('ok')
        mutres = DNA_bin.detecting_mutations(array.array('l',sequence[i][gene[i][j][0]:(gene[i][j][1])]))
    ...
    if (len(gene[i])-1 > 2):
        for j in range((int((len(gene[i]))/2)), -1, -1):
            for k in range((int((len(gene[i]))/2))):     
                ...
                res = (DNA_bin.calculating_matching_score(array.array('l',sequence[i][gene[i][j][0]:gene[i][j][1]]),array.array('l',sequence[i][gene[i][k][0]:gene[i][k][1]])))
                matching.append(res)
    ...
    for c in range((int((len(gene))/2))):     
        for j in range(int((len(gene[i])))):
            for k in range(int((len(gene[c])))):
                ...
                res = (DNA_bin.calculating_matching_score(array.array('l',sequence[i][gene[i][j][0]:gene[i][j][1]]),array.array('l',sequence[c][gene[c][k][0]:gene[c][k][1]])))
                matching.append(res)
                message+="<td>"+str(res)+ "</td> \n</tr>\n"
                f.write(str(res)+"\t")
message+= "</tbody>\n</table>\n\n </html>"  

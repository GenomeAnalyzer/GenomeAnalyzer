    ...
    for c in range((int((len(gene))/2))):     
        for j in range(int((len(gene[i])))):
            for k in range(int((len(gene[c])))):
                res = DNA_bin.calculating_matching_score(array.array('l',sequence[i]),gene[i][j][0],gene[i][j][1]- gene[i][j][0],array.array('l',sequence[c]),gene[c][k][0],gene[c][k][1]- gene[c][k][0])           
                message+="<tr><td>sequence</td>\n"
                f.write("[sequence]=")
                matching.append(res)
                message+="<td>"+str(res)+ "</td> \n</tr>\n"

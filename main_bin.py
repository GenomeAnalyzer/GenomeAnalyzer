...
for file in glob.glob("fastas/*.fasta"):
    ...
    for j in range (len(gene[i])-1):
        #print(str(sequence[i])+" "+str(gene[i][j][0])+" "+str(gene[i][j][1]- gene[i][j][0]))
        seq = DNA_bin.get_piece_binary_array(array.array('I',sequence[i]),gene[i][j][0],gene[i][j][1]- gene[i][j][0])
        print("sequence = "+str(seq))

        message += "<tr><td>"+str(seq)+ "</td>\n"
        res = (DNA_bin.generating_mRNA(array.array('l',seq)))
        if res:
        ...
        res2 =  DNA_bin.generating_amino_acid_chain(array.array('l',seq))
        ...
        if res2:
            ...
            message+="<td> none </td>\n"            
        #print('ok')
        mutres = DNA_bin.detecting_mutations(array.array('l',seq))
